...
"Emission Inventory Workflow" is an application designed to prepare area and point source GHG emission inputs for a high-resolution model based on the HARMONIE coordinate system, such as DALES.
...
The input files for this workflow consist of annual raw point source emission data and 1x1 km (or 5x5 km) emission CSV files. Some of open access emission data can be downloaded from https://data-preview.emissieregistratie.nl/

Output: ncdf files of point source emission and 3-D area emission input.

Too run the application use run script: emiss_prep_workflow_run_script.ipynb 
