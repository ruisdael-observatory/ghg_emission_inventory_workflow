...
from emission_construction_functions import create3Dfromscratch, df2numpy
import os
import glob
import shutil
...
class Emission_3Dfield_creation:
    def __init__(self):
        ...
        self.layer_num_emiss_z = 9  #IMPORTANT parameter: if SNAP has z component, emiss will be placed at some number of model layers (depends on zmax) from the bottom to the stack height
                                    #currently, the plume rise height is set as zmax, so emiss should be devided by number of vertical layers
                                    # to get emiss fraction for each layer, making equal destribution and observing the mass conservation.
                                    ...
                                    # emiss fraction will be calculated right in the model !
                                    #TODO: now, emiss are equally destributed between vertical layers until plume rise height, but in the nature in some layers plume contribution could be higher/lower...
    ...
    def process_emissions(self):
        for isnap in self.snaplist:
            ...
            if self.point_source_refinement:
                ...
                points = create3Dfromscratch(x, y, z, zh, self.crs, isnap, self.input_points + 'point_sources_with_snap_HARM.csv',
                                             savepoints=False)
                

                # --- 3 --- Combine
                speci_point = points.copy()
                speci[:, :, 0] += speci_point

                # Logic for point source refinement
            else:
            ...
            spec_name_cap = self.spec_name.upper()

            fobj = netc.Dataset(self.targetdir + f'{self.spec_name}_{self.year}_{isnap}_{icatname}.nc', 'w')

            fobj.description = f"{spec_name_cap} emissions (year {self.year}, SNAP category {isnap})"
            ...
            var_e = fobj.createVariable(spec_name_cap, 'f8', ('z', 'y', 'x',))
