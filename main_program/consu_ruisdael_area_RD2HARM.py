...
class AreaEmissionsReprojection_Consu:
    def __init__(self):
        self.sourcedir=distin_folder_ruisdael_area_csv2gpkg_new_python_only
        self.snaplist = ['2']  # always 2, because this script is for SNAP 2 only!
        ...
        self.emiss_sc_factor = 10000.   #emission scaling factor:
                                        #10000 because in case of consu input is in 100x100m res
                                        # but not in 1000x1000m res #as in others!
    ...
    def reproject_emissions(self):
        ...
        for snap in self.snaplist:
            ...
            source = inputdir_snap + self.spec_name + '_' + str(self.year) + '_SNAP_2_refined_100m_residual.gpkg'
            destination = rundir + self.spec_name + '_' + str(self.year) + '_SNAP_2_refined_100m_residual.gpkg'

            if os.path.isfile(source):
                shutil.copy(source, destination)
                print(f'Copied: {self.spec_name}_{self.year}_SNAP_2_refined_100m_residual.gpkg')
            ...
            for xmin in xminlist:
                for ymin in yminlist:
                    ...
                    intersection_gdf = gpd.GeoDataFrame(columns=['left', 'bottom', co2_column, 'emis', 'geometry'], crs=self.proj_HARM)
                    ...
                    if not intersection.empty:
                        ...
                        intersection['emis'] = (intersection['geometry'].area / self.emiss_sc_factor) * intersection[co2_column]
                        ...
                        try:
                            intersection[['left', 'bottom', co2_column, 'emis', 'geometry']].to_file(
                        ...
                        intersection_gdf = pd.concat(
                                [intersection_gdf, intersection[['left', 'bottom', co2_column, 'emis', 'geometry']]],
                                ignore_index=True)

                        emis_buff_gdf = intersection_gdf[['left', 'bottom', co2_column, 'emis', 'geometry']].copy()
                        ...
                        # Rename the 'emis' column to co2_column:
                        harm_resi_gdf.rename(columns={'emis': co2_column}, inplace=True)
                        ...
                        harm_resi_filtered_gdf = harm_resi_gdf[harm_resi_gdf[co2_column] > 0].copy()
                        ...
                        harm_resi_filtered_gdf = harm_resi_filtered_gdf[
                            ['id', 'left', 'top', 'right', 'bottom', co2_column, 'geometry']]
            ...
            print(f'SNAP{snap} Consu is done.')
